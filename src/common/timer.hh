...
class timer {
 public:
  ...
  static timer        from_seconds(uint32_t seconds)      { return utils::timer((uint64_t)seconds * 1000000); }
  static timer        from_milliseconds(uint32_t seconds) { return utils::timer((uint64_t)seconds * 1000); }
...
inline std::string format_timediff(timer& t)
{
    std::ostringstream res;
    if ( t.seconds() <= 10 ) {
        res << t.seconds() << " s " << t.usec() << " us";
    } else if ( t.seconds() < 60 ) {
        res << t.seconds() << " s";
    } else {
        int min = (t.seconds() / 60) % 60;
